...
namespace Benjathemaker
{
    public class SimpleGemsAnim : MonoBehaviour
    {
        ...
        public float floatSpeed = initialScale.x;
        private Vector3 initialPosition;
        private float floatTimer;

        public bool isScaling = false;
        public bool useEasingForScaling = false; // Separate toggle for scaling ease
        public float scaleLerpSpeed = initialScale.x; // Speed of scaling transition
        ...
        {
            t *= initialScale.x;
            return t < 0.5f ? 2 * t * t : initialScale.x - Mathf.Pow(-1 * t + 1, 2) / 1;
        }
    }
}
